...
export default function SFACAdmission() {
  ...
  const bookmarkedList = useMemo(() => UNIVERSITIES.filter(u => bookmarks[u.id]), [bookmarks]);
        ...
        <header className="mb-6 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight flex items-center gap-2">
              <School className="size-7" aria-hidden /> SFAC Taguig – College Admissions Hub
            </h1>
            <p className="text-muted-foreground max-w-2xl">Centralized, student-friendly portal for admissions in Manila & Taguig. Save schools, track tasks, and never miss a deadline.</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-2 text-sm text-muted-foreground">
              <CheckCircle2 className="size-4" /> Overall progress
              <Progress value={checklistStats.pct} className="w-40" aria-label={`Overall checklist ${checklistStats.pct}%`} />
