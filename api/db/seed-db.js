...
// Drop old tables if needed (so reseeding works cleanly)
db.exec(`
...
const insertUniversity = db.prepare(`
  INSERT INTO universities (id, name, location, deadline, exam_name, exam_window)
  VALUES (?, ?, ?, ?, ?, ?)
`);

const insertProgram = db.prepare(`
  INSERT INTO programs (university_id, program)
  VALUES (?, ?)
`);

const insertRequirement = db.prepare(`
  INSERT INTO requirements (university_id, requirement)
  VALUES (?, ?)
`);

const insertLink = db.prepare(`
  INSERT INTO links (university_id, type, url)
  VALUES (?, ?, ?)
`);
...
for (const uni of universities) {
  ...
  uni.programs.forEach((p) => insertProgram.run(uni.id, p));
  uni.requirements.forEach((r) => insertRequirement.run(uni.id, r));
  Object.entries(uni.links).forEach(([type, url]) =>
    insertLink.run(uni.id, type, url)
  );
}
